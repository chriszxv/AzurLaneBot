...
def checkGameState():
    location = localeImage(
        '.\\images\\air_raid_drills_with_essex\\essex', confidence=0.7)
    if location is not None:
...
def handlePreCombatState():
    ...
    print('Current EX: ' + str(current_ex_count) +
          '/' + str(maximum_ex_count) + ' time(s)')
    print('Current Hard: ' + str(current_hard_count) +
          '/' + str(maximum_hard_count) + ' time(s)')
    print('Current Normal: ' + str(current_normal_count) +
          '/' + str(maximum_normal_count) + ' time(s)')
    print('Current Easy: ' + str(current_easy_count) +
          '/' + str(maximum_easy_count) + ' time(s)')

    if current_ex_count < maximum_ex_count:
        print('handle ex chapter:')
        current_ex_count = current_ex_count + 1
        clickImage('.\\images\\air_raid_drills_with_essex\\ex', confidence=0.7)

    elif current_hard_count < maximum_hard_count:
        print('handle hard chapter:')
        current_hard_count = current_hard_count + 1
        clickImage('.\\images\\air_raid_drills_with_essex\\hard',
                   confidence=0.7)

    elif current_normal_count < maximum_normal_count:
        print('handle normal chapter:')
        current_normal_count = current_normal_count + 1
        clickImage('.\\images\\air_raid_drills_with_essex\\normal',
                   confidence=0.7)

    elif current_easy_count < maximum_easy_count:
        print('handle easy chapter:')
        current_easy_count = current_easy_count + 1
        clickImage('.\\images\\air_raid_drills_with_essex\\easy',
                   confidence=0.7)
